...
from train_parser import generate_parser, generate_config, generate_loss_config      #这儿的config这个得改，它得用到word_embed生成的东西
...
def run(args):
    ...
    train_dataset = NestedNERDataset(args.version, 'train', args.bert_name_or_path, args.truncate_length, args.schema, args.use_context, args.token_schema, args.soft_iou, bert_embed=train_bert_embed)    #这里注意下，这个dataset重写后会直接调用getitem函数，所以要看getitem才知道其数据结构
    ...
    dev_strict, dev_relax, new_dev_strict = decode(dev_dataloader, model, args, ema)
    test_strict, test_relax, new_test_strict = decode(test_dataloader, model, args, ema)
    
    write_predict(new_dev_strict, os.path.join(output_path, 'dev_predict.txt'))
    write_predict(new_test_strict, os.path.join(output_path, 'test_predict.txt'))
...
def train_one_epoch(model, steps, train_dataloader, dev_dataloader, test_dataloader, optimizer, scheduler, writer, args, epoch_idx, ema):
    ...
    dev_strict, dev_relax, new_dev_strict = decode(dev_dataloader, model, args, ema)        #relax是啥没看出来，他写的都是空，不知道干啥的
    write_predict(new_dev_strict, os.path.join(output_path, f'dev_{epoch_idx}_predict.txt'))    #这儿是写，就是说dev-strict里是span,label的list
    test_strict, test_relax, new_test_strict = decode(test_dataloader, model, args, ema)
    write_predict(new_test_strict, os.path.join(output_path, f'test_{epoch_idx}_predict.txt'))
