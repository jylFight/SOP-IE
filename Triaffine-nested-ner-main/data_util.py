...
class NestedNERDataset(Dataset):
    def __init__(self, version, mode, tokenizer, truncate_length=128, schema="span", use_context=True,
        ...
        self.file_path = self._get_file_path(self.version, self.mode)       #这个函数也要自己改，因为没有我设置的version
        self.df = self._load_file(self.file_path)
        self.type2id = self._get_type(self.version)                         #这儿我可能得改一下，因为要改成自己的ner标签，即head,tail,both
        self.id2type = {idx: tp for tp, idx in self.type2id.items()}        #这儿同理

        self.truncate_length = truncate_length
        self.use_context = use_context

        self.word2id, self.id2word = self._load_vocab(self.version)         #这儿得用word_embed得到
    ...
    def _get_file_path(self, version, mode):
        return f"data/{version}/{mode}_sample.json"        #这个就是最原始的
    ...
    def _get_type(self, version):
        ...
        if version.find('LSOIE') >= 0 or version.find('CaRB') >= 0 :          #这两句是我后加的，LSOIE-SCI也符合
            type_list = ['both', 'head', 'tail']
    ...
    def __getitem__(self, index):
        line = self.df[index]
        tokens = line['tokens']
        if self.use_context:                   #所以不用use_context的时候就可以不用这俩了
        ...
        for t in self.tokenizer_list:                                         #这个ce看了代码意思是一个词是否被tokenize拆分了，做的记录
