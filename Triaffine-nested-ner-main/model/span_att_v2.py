...
class SpanAttModelV2(nn.Module):
    ...
    def predict(self, input_ids, attention_mask, ce_mask, token_type_ids, subword_group,
        ...
        #new_span_score维度是b,max_span,label_num(包括None)，里面的数值需要softmax才能得到概率！注意修改一下！
        
        bsz = input_ids.size(0)               #batch_size
        ...
        for i in range(bsz):
            ...
            new_score = torch.softmax(new_span_score,dim=-1)
            score=new_score[i,use_idx[i],cl]        
            result.append([[x[j], y[j], cl[j], score[j]] for j in range(x.size(0))])
