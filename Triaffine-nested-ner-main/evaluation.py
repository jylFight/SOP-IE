...
def decode(dataloader, model, args, ema=None):
    ...
    model.eval()
    predict_labels, new_predict_labels = [], []
    
    if ema is None:
        with torch.no_grad():
            for batch in dataloader:
                inputs = prepare_input(batch, args, train=False)
                result = model.predict(**inputs)    #所以想要输出概率，还是得看这个model.predict

                for i in range(batch[0].size(0)):
                    predict_label = list(set([f'{tint(l[0])},{tint(l[1]) + 1} {dataloader.dataset.id2type[tint(l[2])]}' for l in result[i]]))
                    #根据结果来看，这里面输出的是span[0],span[1]和id
                    predict_labels.append(predict_label)
                    new_predict_label = list(set([f'{tint(l[0])},{tint(l[1]) + 1} {dataloader.dataset.id2type[tint(l[2])]}, {round(float(l[3]),3)}' for l in result[i]]))
                    #根据结果来看，这里面输出的是span[0],span[1]和id
                    new_predict_labels.append(new_predict_label)
    else:
        with torch.no_grad():
    ...
    return predict_labels, [] , new_predict_labels
...
def metric(dataset, predict_labels_0, suffix=None):
    ...
    for i in range(len(dataset)):
        ...
        for l in predict_label:                   #这儿也有问题，我改了之后原来的label不止这仨东西,还有score,所以匹配不上，得改一下
            if l in true_label:
                correct += 1
