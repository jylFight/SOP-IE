...
def generate_parser():
    ...
    parser.add_argument("--version", type=str, default="genia",
                        choices=["ace05", "ace04", "genia91", "kbp","LSOIE","LSOIE-SCI","CaRB","LSOIE-GPT-IE"],
    ...
    parser.add_argument("--ema", type=float, default=0.)              #查了一下是加强模型鲁棒性的，默认为不用，所以不管就行
    ...
    parser.add_argument("--use_context", action="store_true")       #store_true就是如果给出了就是true，没给就是false
...
def generate_config(args):
    ...
    if args.word:
        ...
        if args.version=="LSOIE":                #为啥padding_idx还不一样呢！？这个我不理解我说实话，我只能按照它是word的最大数来算，不然理解不了
            if not args.word_embed:
                word_embedding_config['path'] = 'data/ace04/wiki.npy'
                word_embedding_config['dim'] = 300
            elif args.word_embed == "glove":
                ...
                word_embedding_config['path'] = 'data/LSOIE/cc.npy'
                word_embedding_config['dim'] = 300                
            word_embedding_config['padding_idx'] = 39751    
        if args.version.find("LSOIE-SCI") >= 0:
            word_embedding_config['path'] = 'data/LSOIE-SCI/cc.npy'
            word_embedding_config['dim'] = 300   
            word_embedding_config['padding_idx'] = 44474
        if args.version.find("CaRB") >= 0:
            word_embedding_config['path'] = 'data/CaRB/cc.npy'
            word_embedding_config['dim'] = 300   
            word_embedding_config['padding_idx'] = 91311
        if args.version.find("LSOIE-GPT-IE") >= 0:
            word_embedding_config['path'] = 'data/LSOIE-GPT-IE/cc.npy'
            word_embedding_config['dim'] = 300   
            word_embedding_config['padding_idx'] = 9256
    ...
    if args.char:
        ...
        if args.version=="LSOIE":
            char_embedding_config['padding_idx'] = 513
        if args.version.find("LSOIE-SCI") >= 0:
            char_embedding_config['padding_idx'] = 502
        if args.version.find("CaRB") >= 0:
            char_embedding_config['padding_idx'] = 645
        if args.version.find("LSOIE-GPT-IE") >= 0:
            char_embedding_config['padding_idx'] = 130
    ...
    if args.pos:
        ...
        if args.version.find("kbp") >= 0:
            pos_embedding_config['padding_idx'] = 46
        if args.version=="LSOIE" or args.version=="LSOIE-GPT-IE":
            pos_embedding_config['padding_idx'] = 57
        if args.version.find("LSOIE-SCI") >= 0:
            pos_embedding_config['padding_idx'] = 57
        if args.version.find("CaRB") >= 0:
            pos_embedding_config['padding_idx'] = 58
